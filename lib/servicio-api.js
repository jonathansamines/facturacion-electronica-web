...
export const buscarProducto = async ({ req, busqueda, tipoDocumento } = {}) => {
...
export const obtenerTipoDocumentoPorId = async ({ req, idTipoDocumento } = {}) => {
  const respuesta = await servicio.get(`/tipos-documento/${idTipoDocumento}`);

  return respuesta.data;
};

export const obtenerTiposProducto = async ({ req, tipoDocumento } = {}) => {
  const respuesta = await servicio.get('/tipos-producto', {
    params: {
      tipo_documento: tipoDocumento,
      ...credenciales(req)
    }
