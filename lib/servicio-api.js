...
export const obtenerFacturas = async ({ req } = {}) => {
  const respuesta = await servicio.get('/facturas', credenciales(req));
...
export const generarFactura = async ({ req, factura } = {}) => {
  const respuesta = await servicio.post(`/facturas`, factura, credenciales(req));
