...
export function validarCUI(cui) {
  ...
  if (!cuiRegExp.test(cui)) {
    return {
      error: 'Formato de CUI no es válido'
    };
  }
  ...
  if (departamento === 0 || municipio === 0) {
    return {
      error: 'CUI con código de municipio o departamento inválido.'
    };
  }

  if (departamento > munisPorDepto.length) {
    return {
      error: 'CUI con código de departamento inválido.'
    };
  }

  if (municipio > munisPorDepto[departamento -1]) {
    return {
      error: 'CUI con código de municipio inválido.'
    };
  }
